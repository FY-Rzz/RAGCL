...
from Main.sort import sort_5fold_dataset
...
if __name__ == '__main__':
    ...
    # 数据集划分
    # sort_5fold_dataset(label_source_path, label_dataset_path, k_shot=k, n_splits=5)
    print("数据集划分完成！")
    ...
    for fold in range(5):
        fold_train_path = osp.join(label_dataset_path, f'fold_{fold}', 'train')
        fold_test_path = osp.join(label_dataset_path, f'fold_{fold}', 'test')
        train_raw_path = osp.join(fold_train_path, 'raw')
        test_raw_path = osp.join(fold_test_path, 'raw')

        # # 获取训练和测试数据文件
        # train_files = [osp.join(train_raw_path, f) for f in os.listdir(fold_train_path) if f.endswith('.json')]
        # test_files = [osp.join(test_raw_path, f) for f in os.listdir(fold_test_path) if f.endswith('.json')]

        # # 检查数据集是否为空
        # if len(train_files) == 0 or len(test_files) == 0:
        #     raise ValueError(f"数据集为空！fold_{fold}")

        # 初始化 log_record
        write_log(log, f'run:{fold}')
        log_record = {'run': fold, 'val accs': [], 'test accs': [], 'test precs': [], 'test recs': [], 'test f1s': [],
                      'test micro metric': [], 'test macro metric': []}

        # 数据加载器
        train_dataset = TreeDataset(fold_train_path, word_embedding, word2vec, centrality, undirected)
        test_dataset = TreeDataset(fold_test_path, word_embedding, word2vec, centrality, undirected)
        ...
        val_error, log_info, log_record = test_and_log(model, test_loader, test_loader, num_classes,
        ...
        for epoch in range(1, epochs + 1):
            ...
            val_error, log_info, log_record = test_and_log(model, test_loader, test_loader, num_classes, device, epoch,
        ...
        # 记录当前折叠的测试准确率
        all_test_accs.append(log_record['test accs'][-1])
        ...
        # 删除临时文件夹
        os.system(f'rm -r {fold_train_path}')
        os.system(f'rm -r {fold_test_path}')
    # 计算5个折叠的平均测试准确率
    mean_acc = round(np.mean(all_test_accs), 3)
    print(f"5折交叉验证的平均测试准确率: {mean_acc}")